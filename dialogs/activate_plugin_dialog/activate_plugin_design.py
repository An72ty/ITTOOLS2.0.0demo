# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'activate_plugin_design.ui'
#
# Created by: PyQt5 UI code generator 5.15.9
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_Dialog(object):
    def setupUi(self, Dialog):
        Dialog.setObjectName("Dialog")
        Dialog.resize(316, 221)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../images/icons/touch.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Dialog.setWindowIcon(icon)
        Dialog.setStyleSheet("background-color: #0d1117;\n"
"font: 25px \"Verdana\";\n"
"")
        self.hint = QtWidgets.QLabel(Dialog)
        self.hint.setGeometry(QtCore.QRect(20, 70, 301, 31))
        self.hint.setStyleSheet("QLabel {\n"
"    color: white;\n"
"    font-size: 15px;\n"
"}")
        self.hint.setObjectName("hint")
        self.title = QtWidgets.QLabel(Dialog)
        self.title.setGeometry(QtCore.QRect(20, 20, 231, 41))
        self.title.setStyleSheet("QLabel {\n"
"    color: white;\n"
"    font-size: 30px;\n"
"}")
        self.title.setObjectName("title")
        self.install = QtWidgets.QPushButton(Dialog)
        self.install.setGeometry(QtCore.QRect(220, 100, 35, 35))
        self.install.setStyleSheet("QPushButton {\n"
"    border: 0px;\n"
"    color: white;\n"
"    font: 25px \"Verdana\";\n"
"    border-radius: 10px;\n"
"}\n"
"\n"
"QPushButton::hover {\n"
"    background-color: #5a6677;\n"
"}\n"
"")
        self.install.setText("")
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("../../images/icons/touch.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.install.setIcon(icon1)
        self.install.setIconSize(QtCore.QSize(30, 30))
        self.install.setObjectName("install")
        self.cancel = QtWidgets.QPushButton(Dialog)
        self.cancel.setGeometry(QtCore.QRect(260, 100, 35, 35))
        self.cancel.setStyleSheet("QPushButton {\n"
"    border: 0px;\n"
"    color: white;\n"
"    font: 25px \"Verdana\";\n"
"    border-radius: 10px;\n"
"}\n"
"\n"
"QPushButton::hover {\n"
"    background-color: #5a6677;\n"
"}\n"
"")
        self.cancel.setText("")
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap("../../images/icons/exit.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.cancel.setIcon(icon2)
        self.cancel.setIconSize(QtCore.QSize(30, 30))
        self.cancel.setObjectName("cancel")
        self.comboBox = QtWidgets.QComboBox(Dialog)
        self.comboBox.setGeometry(QtCore.QRect(20, 110, 191, 22))
        self.comboBox.setStyleSheet("QComboBox {\n"
"    background-color: white;\n"
"    border-width: 0px;\n"
"}\n"
"")
        self.comboBox.setObjectName("comboBox")

        self.retranslateUi(Dialog)
        QtCore.QMetaObject.connectSlotsByName(Dialog)

    def retranslateUi(self, Dialog):
        _translate = QtCore.QCoreApplication.translate
        Dialog.setWindowTitle(_translate("Dialog", "Install plugin"))
        self.hint.setText(_translate("Dialog", "Select plugin"))
        self.title.setText(_translate("Dialog", "Activate plugin"))


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    Dialog = QtWidgets.QDialog()
    ui = Ui_Dialog()
    ui.setupUi(Dialog)
    Dialog.show()
    sys.exit(app.exec_())
